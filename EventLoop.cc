...
int createEventfd()
{
    ...
    if (evtfd < 0)
    {
        LOG(LoggerLevel::FATAL,"eventfd error:%d\n", errno);
    }
    return evtfd;
}
...
{
    LOG(LoggerLevel::DEBUG,"EventLoop created %p in thread %d\n", this, threadId_);
    if (t_loopInThisThread)
    {
        LOG(LoggerLevel::FATAL,"Another EventLoop %p exists in this thread %d\n", t_loopInThisThread, threadId_);
    }
    else
    {
...
void EventLoop::loop()
{
    ...
    LOG(LoggerLevel::INFO,"EventLoop %p start looping\n", this);
    ...
    LOG(LoggerLevel::INFO,"EventLoop %p stop looping.\n", this);
    looping_ = false;
}
...
void EventLoop::handleRead()
{
    ...
    if (n != sizeof(one))
    {
        LOG(LoggerLevel::ERROR,"EventLoop::handleRead() reads %lu bytes instead of 8\n", n);
    }
}
...
void EventLoop::wakeup()
{
    ...
    if (n != sizeof(one))
    {
        LOG(LoggerLevel::ERROR,"EventLoop::wakeup() writes %lu bytes instead of 8\n", n);
    }
}
...
Timer* EventLoop::runAt(Timestamp when, TimerCallback pRun)
{
    return _pTimerQueue->addTimer(::move(pRun), when, 0.0);
}

Timer* EventLoop::runAfter(double delay, TimerCallback pRun)
{
    std::cout << Timestamp::nowAfter(delay).toString() << std::endl;
    return _pTimerQueue->addTimer(::move(pRun), Timestamp::nowAfter(delay), 0.0);
}

Timer* EventLoop::runEvery(double interval, TimerCallback pRun)
{
    return _pTimerQueue->addTimer(::move(pRun), Timestamp::nowAfter(interval), interval);
}

void EventLoop::cancelTimer(Timer* timer)
{
    _pTimerQueue->cancelTimer(timer);
}
