...
static int createNonblocking()
{
    ...
    if (sockfd < 0)
    {
        LOG(LoggerLevel::FATAL,"%s:%s:%d listen socket create err:%d\n", __FILE__, __FUNCTION__, __LINE__, errno);
    }
    return sockfd;
}
...
{
    LOG(LoggerLevel::INFO,"do void Acceptor::handleRead()  %d",51);
    ...
    {
        LOG(LoggerLevel::ERROR,"%s:%s:%d accept err:%d\n", __FILE__, __FUNCTION__, __LINE__, errno);
        if (errno == EMFILE)
        {
            LOG(LoggerLevel::ERROR,"%s:%s:%d sockfd reached limit\n", __FILE__, __FUNCTION__, __LINE__);
        }
    }
}