...
#define LOG_INIT(logdir, lev) \
    do { \
        AsyncLogger::GetInstance()->Init(logdir, lev); \
    } while (0)

#define LOG(level, fmt, ...) \
    do {  \
        if(AsyncLogger::GetInstance()->GetLevel() <= level) {  \
            AsyncLogger::GetInstance()->Append(level, __FILE__, __LINE__, __FUNCTION__, fmt, __VA_ARGS__);  \
        }  \
    } while (0)  
