...
void AsyncLogger::Init(const char* logdir, LoggerLevel lev) {
    //alloc logbuf
    ...
    //create flush thread
    flushthread = std::thread(&AsyncLogger::Flush, this);
    return ;
}

void AsyncLogger::Append(int level, const char *file, int line, const char *func, const char *fmt, ...) {
    //单行日志
