...
static EventLoop *CheckLoopNotNull(EventLoop *loop)
{
    if (loop == nullptr)
    {
        LOG(LoggerLevel::FATAL,"%s:%s:%d mainLoop is null!\n", __FILE__, __FUNCTION__, __LINE__);
    }
    return loop;
}
    ...
    , highWaterMark_(64 * 1024 * 1024) // 64M
{
    ...
    LOG(LoggerLevel::INFO,"TcpConnection::ctor[%s] at fd=%d\n", name_.c_str(), sockfd);
    socket_->setKeepAlive(true);
}

TcpConnection::~TcpConnection()
{
    LOG(LoggerLevel::INFO,"TcpConnection::dtor[%s] at fd=%d state=%d\n", name_.c_str(), channel_->fd(), (int)state_);
}

void TcpConnection::send(const std::string &buf)
{
    LOG(LoggerLevel::INFO,"TcpConnection::send",NULL);
...
{
    LOG(LoggerLevel::INFO,"TcpConnection::sendInLoopnum: %d",80);
    ssize_t nwrote = 0;
    ...
    {
        LOG(LoggerLevel::ERROR,"disconnected, give up writingnum: %d",87);
    }
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        else // nwrote < 0
        {
            ...
            {
                LOG(LoggerLevel::ERROR,"TcpConnection::sendInLoop num: %d",109);
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    {
        errno = savedErrno;
        LOG(LoggerLevel::ERROR,"TcpConnection::handleReadnum: %d",200);
        handleError();
    }
}
...
void TcpConnection::handleWrite()
{
    if (channel_->isWriting())
    {
        ...
        {
            LOG(LoggerLevel::ERROR,"TcpConnection::handleWritenum: %d",231);
        }
    }
    else
    {
        LOG(LoggerLevel::ERROR,"TcpConnection fd=%d is down, no more writing", channel_->fd());
    }
}

void TcpConnection::handleClose()
{
    LOG(LoggerLevel::INFO,"TcpConnection::handleClose fd=%d state=%d\n", channel_->fd(), (int)state_);
    setState(kDisconnected);
    channel_->disableAll();
...
{
    LOG(LoggerLevel::INFO,"TcpConnection::handleError()num: %d",253);
    int optval;
    ...
    LOG(LoggerLevel::ERROR,"TcpConnection::handleError name:%s - SO_ERROR:%d\n", name_.c_str(), err);
}